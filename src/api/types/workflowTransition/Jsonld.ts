...
*/
export type WorkflowTransitionJsonld = (HydraItemBaseSchema & {
    /**
     * @description Null = applies to any tracker that doesn\'t have its own rules\n(the baseline workflow). When a per-tracker override exists,\nthe baseline is ignored for that tracker.
    ...
    */
    toStatus?: string;
    /**
     * @description Stored as JSON list of WorkspaceMemberRole `value` strings.
     * @type array,null
    */
    allowedRoles?: string[] | null;
    /**
     * @description Optional human-facing label shown next to the transition button —\n\"Submit for review\", \"Re-open\", \"Send to QA\". Omitted = the SPA\njust labels the button with the target status name.
     * @type string,null
    */
    label?: string | null;
    /**
